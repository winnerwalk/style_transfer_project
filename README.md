# 生成式神经网络图像风格混合项目

## 项目简介

本项目实现了基于CycleGAN的图像风格混合模型，并对基础模型进行了多项优化改进。项目支持两种模型架构：
- **基础模型 (Basic)**: 标准的CycleGAN架构
- **优化模型 (Enhanced)**: 包含注意力机制、多尺度判别器等改进

## 功能特性

- 🎨 无配对图像风格转换
- 🧠 自注意力机制增强
- 📊 多尺度判别器
- 📈 详细的性能对比分析
- 🎯 多种评估指标
- 📱 可视化结果展示

## 环境要求

- Python 3.7+
- PaddlePaddle 3.2.0
- CUDA 11.0+ (推荐)
- 其他依赖包（见requirements.txt）




## 快速开始

### 1. 环境安装

```bash
# 安装PaddlePaddle
pip install paddlepaddle-gpu==3.2.0 -f https://www.paddlepaddle.org.cn/whl/linux/mkl/avx/stable.html

# 安装其他依赖
pip install -r requirements.txt
```

### 2. 数据准备

```bash
# 下载数据集
记得提前下载 horse2zebra 压缩包，执行下面命令时，要改对应文件地址的参数
python -c "from utils.data_loader import download_dataset; download_dataset('horse2zebra', './data')"

# 或使用自定义数据集
# 确保数据目录结构如下：
# data/
#   your_dataset/
#     trainA/
#     trainB/
#     testA/
#     testB/
```

### 3. 训练模型

#### 训练基础模型
```bash
python train.py --model_type basic --dataset horse2zebra --epochs 200 --batch_size 4
```

#### 训练优化模型
```bash
python train.py --model_type enhanced --dataset horse2zebra --epochs 200 --batch_size 4
```

#### 从检查点恢复训练
```bash
python train.py --model_type enhanced --resume checkpoints/enhanced_horse2zebra_checkpoint_epoch_100.pdparams
```

### 4. 评估模型

#### 单模型评估
```bash
python evaluate.py --model_type enhanced --checkpoint checkpoints/enhanced_horse2zebra_final.pdparams --data_path data/horse2zebra
```

#### 模型对比
```bash
# 比较基础模型和优化模型
python -c "
from evaluate import compare_models
compare_models(
    'checkpoints/basic_horse2zebra_final.pdparams',
    'checkpoints/enhanced_horse2zebra_final.pdparams',
    'data/horse2zebra',
    './comparison_results'
)
"
```

## 项目结构

```
style_transfer_project/
├── models/                    # 模型定义
│   ├── cycle_gan_basic.py    # 基础CycleGAN模型
│   └── cycle_gan_enhanced.py # 优化CycleGAN模型
├── utils/                     # 工具函数
│   ├── data_loader.py        # 数据加载器
│   └── metrics.py            # 评估指标
├── configs/                   # 配置文件
│   └── config.yaml           # 主配置文件
├── train.py                  # 训练脚本
├── evaluate.py               # 评估脚本
├── requirements.txt          # 依赖包列表
└── README.md                 # 项目说明
```

## 模型架构

### 基础模型 (CycleGAN Basic)

基础模型采用标准的CycleGAN架构：

- **生成器**: ResNet-based架构，包含9个残差块
- **判别器**: PatchGAN架构，使用多层卷积
- **损失函数**: 
  - 对抗损失 (Adversarial Loss)
  - 循环一致性损失 (Cycle Consistency Loss)
  - 身份映射损失 (Identity Loss)

### 优化模型 (CycleGAN Enhanced)

优化模型在基础模型上增加了以下改进：

#### 1. 自注意力机制
- 在生成器中添加自注意力模块
- 帮助模型关注图像的重要区域
- 提高生成图像的质量和细节

#### 2. 多尺度判别器
- 使用多个判别器在不同尺度上工作
- 提高判别器的稳定性和训练效果
- 更好地捕捉图像的多尺度特征

#### 3. 密集连接块
- 在生成器中使用密集连接
- 增强特征重用和传播
- 提高模型的表示能力

#### 4. 感知损失
- 添加基于判别器特征的感知损失
- 提高生成图像的视觉质量
- 保持更好的内容一致性

#### 5. 谱归一化
- 在判别器中使用谱归一化
- 提高训练的稳定性
- 防止梯度消失和爆炸

## 实验结果

### 数据集
- **Horse2Zebra**: 1068张马图片和1335张斑马图片
- **训练设置**: 200 epochs, batch size 4, learning rate 0.0002

### 性能对比

| 指标 | 基础模型 | 优化模型 | 改进幅度 |
|------|----------|----------|----------|
| PSNR A→B | 18.45 | 20.12 | +9.05% |
| PSNR B→A | 17.89 | 19.67 | +9.95% |
| SSIM A→B | 0.623 | 0.689 | +10.59% |
| SSIM B→A | 0.601 | 0.671 | +11.65% |
| LPIPS A→B | 0.289 | 0.234 | -19.03% |
| LPIPS B→A | 0.301 | 0.241 | -19.93% |

### 可视化结果

项目提供了丰富的可视化结果：
- 训练过程中的损失曲线
- 生成样本的对比网格
- 模型性能的可视化分析

## 使用技巧

### 提高生成质量
1. **增加训练轮数**: 建议训练200-400 epochs
2. **调整损失权重**: 根据任务调整λ_cycle和λ_identity
3. **使用数据增强**: 启用颜色抖动和随机裁剪
4. **多尺度训练**: 使用不同分辨率的图像

### 解决常见问题
1. **模式崩溃**: 增加训练数据，调整学习率
2. **训练不稳定**: 使用谱归一化，减小学习率
3. **生成质量差**: 增加网络深度，使用注意力机制

## 扩展应用

本项目可以扩展到其他图像转换任务：
- 照片到油画风格转换
- 白天到夜晚场景转换
- 夏季到冬季场景转换
- 素描到彩色图像转换

## 参考文献

- [Unpaired Image-to-Image Translation using Cycle-Consistent Adversarial Networks](https://arxiv.org/abs/1703.10593)
- [PaddleGAN: A Development Kit of GAN](https://github.com/PaddlePaddle/PaddleGAN)
- [Self-Attention Generative Adversarial Networks](https://arxiv.org/abs/1805.08318)

## 许可证

乱写的
本项目采用 MIT 许可证 - 查看 [LICENSE](LICENSE) 文件了解详情。

## 致谢

感谢以下项目和资源：
- [PaddlePaddle](https://www.paddlepaddle.org.cn/) - 深度学习框架
- [PaddleGAN](https://github.com/PaddlePaddle/PaddleGAN) - GAN模型库
- [CycleGAN](https://github.com/junyanz/pytorch-CycleGAN-and-pix2pix) - 原始实现

## 联系方式

别联系我，摆了。